...
internal static class ShiftAndRollOpCodeProcessors
{
    ...
    public static void Process_ROL(State processorState, Memory memory, ushort? address)
    {
        var value = address == null ? processorState.A : memory.ReadByte(address.Value);

        value = (byte)((byte)(value << 1) | (byte)(value >> 7));
        var carry = (value & 0b0000001) == 1;
        value = (byte)(processorState.C ? value | 1 : value & 0b11111110);
        if(address!=null) memory.Write(address.Value, value);
    ...
    public static void Process_ROR(State processorState, Memory memory, ushort? address)
    {
        var value = address == null ? processorState.A : memory.ReadByte(address.Value);
    ...
    public static void Process_LSR(State processorState, Memory memory, ushort? address)
    {
        var value = address == null ? processorState.A : memory.ReadByte(address.Value);
    ...
    public static void Process_ASL(State processorState, Memory memory, ushort? address) 
    {
        if (address == null)
        {
            ...
            Process_ASL_M(processorState, memory, address.Value);
    }

    private static void Process_ASL_M(State processorState, Memory memory, ushort address)
    {
